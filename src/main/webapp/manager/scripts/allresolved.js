...
let input = document.getElementById('allresolved');
...
if (userString === null) {
    ...
    if (currentUser != null) {
        welcome.innerHTML = "Welcome " + currentUser.username + " to all employee resolved requests";
    }
}
allresolved();
...
function allresolved() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (this.readyState === 4 && this.status === 204) {
            console.log("Failed");
            let childDiv = document.getElementById("allresolved");
            childDiv.textContent = "No resolved requests found";
        }
    }

    xhr.open("GET", "http://localhost:8080/project-1/resolvedrequests");
    xhr.send();
}
...
function renderHTML(data) {
	for (var i = 0; i < data.length; i++) {
		...
		input.append("Resolved: " + data[i].resolved);
		input.append(document.createElement("br"));
